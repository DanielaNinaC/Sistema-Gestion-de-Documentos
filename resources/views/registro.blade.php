...
<form action="{{ route('documentos.store') }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-xl shadow space-y-4">
    @csrf

    <div>
        <label class="block font-semibold">Código</label>
        <input id="codigo-empleado" type="text" name="codigo" value="{{ old('codigo') }}" class="w-full border p-2 rounded-xl" required>
    </div>

    <div>
        <label class="block font-semibold">Nombre del Empleado</label>
        <input id="nombre-empleado" type="text" name="empleado" value="{{ old('empleado') }}" class="w-full border p-2 rounded-xl bg-gray-100" readonly>
    ...
    <div id="documentos-container" class="space-y-4">
        @for ($i = 0; $i < $documentosCount; $i++)
            <div class="documento-item border rounded-xl p-4 bg-gray-50 space-y-3">
                ...
                <div>
                    ...
                    <input type="file" name="archivo[]" class="w-full border p-2 rounded-xl" accept=".pdf,.doc,.docx" required>
                </div>
            </div>
        @endfor
    </div>
    ...
    <div class="flex flex-col items-start gap-3 pt-2">
        <button type="button" id="agregar-documento" class="bg-gray-700 text-white px-4 py-2 rounded-xl" {{ $sinTipos ? 'disabled' : '' }}>
            Registrar un documento mas
        </button>
        <button class="bg-blue-600 text-white px-4 py-2 rounded-xl" {{ $sinTipos ? 'disabled' : '' }}>
            Guardar Documentos
        </button>
    </div>
